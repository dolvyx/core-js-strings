...
function isPalindrome(str) {
  const chistoe = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const obratnoe = chistoe.split('').reverse().join('');
  if (chistoe === obratnoe) {
    return true;
  }
  return false;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrSentence.length; i += 1) {
    if (arrSentence[i].length > longest.length) {
...
function reverseWords(str) {
  ...
  const revArr = strArr.map((item) => reverseString(item));
  return revArr.join(' ');
}
...
function encodeToRot13(str) {
  const inin = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const out = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const ii = inin.indexOf(str[i]);
    if (ii === -1) {
      result += str[i];
    } else {
      result += out[ii];
